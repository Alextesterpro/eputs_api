...
class DailyWorksheetsAPI:
    """API для работы с ежедневными рабочими ведомостями"""

    # Базовый URL для ведомостей
    BASE_WORKSHEETS_URL = "http://91.227.17.139/services/react/api/daily-worksheet"
    ...
    HEADERS = {
        ...
        "Referer": "http://91.227.17.139/react/dictionaries/daily-worksheet",
    ...
    @classmethod
    def create_worksheet(cls, data, headers=None):
        """Создание новой ежедневной рабочей ведомости"""
        if headers is None:
            headers = cls.HEADERS

        url = cls.BASE_WORKSHEETS_URL
        response = requests.post(url, json=data, headers=headers, timeout=10)
        return response

    @classmethod
    def get_worksheets_list(cls, params=None, headers=None):
        """Получение списка ежедневных рабочих ведомостей"""
        if headers is None:
            headers = cls.HEADERS
    ...
    @classmethod
    def get_worksheet_by_id(cls, worksheet_id, headers=None):
        """Получение ведомости по ID"""
        if headers is None:
            headers = cls.HEADERS

        url = f"{cls.BASE_WORKSHEETS_URL}/{worksheet_id}"
        response = requests.get(url, headers=headers, timeout=10)
        return response

    @classmethod
    def update_worksheet(cls, worksheet_id, data, headers=None):
    ...
    @classmethod
    def delete_worksheet(cls, worksheet_id, headers=None):
        """Удаление ведомости"""
        if headers is None:
            headers = cls.HEADERS
