...
#     """Тесты для получения списка ключевых слов"""
...
#     def test_get_keywords_list_default_params(self):
#         """Тест получения списка ключевых слов с параметрами по умолчанию"""
#         response = KeywordsAPI.get_keywords_list()
        
#         # Проверяем статус код (может быть 502 из-за проблем с сервером)
#         assert response.status_code in [KEYWORD_STATUS_CODES["OK"], 502], \
...
#             print(f"Получен список ключевых слов с параметрами по умолчанию. Количество: {len(json_data['data'])}")
#         else:
#             print("Сервер вернул 502 ошибку (проблемы с сервером)")

#     def test_get_keywords_list_with_pagination(self):
#         """Тест получения списка ключевых слов с пагинацией"""
#         params = {
#             'page': DEFAULT_KEYWORD_PAGE,
#             'limit': DEFAULT_KEYWORD_LIMIT
#         }
        
#         response = KeywordsAPI.get_keywords_list(params)
...
#     @handle_server_timeout
#     def test_get_keywords_list_with_custom_limit(self):
#         """Тест получения списка ключевых слов с пользовательским лимитом"""
#         custom_limit = 10
...
#     @handle_server_timeout
#     def test_get_keywords_list_with_max_limit(self):
#         """Тест получения списка ключевых слов с максимальным лимитом"""
#         params = {'limit': MAX_KEYWORD_LIMIT}
...
#     def test_get_keywords_list_with_invalid_limit(self):
#         """Тест получения списка ключевых слов с неверным лимитом"""
#         params = {'limit': 999999}  # Слишком большой лимит
        
#         response = KeywordsAPI.get_keywords_list(params)
        
#         # API может вернуть ошибку или ограничить лимит
#         assert response.status_code in [KEYWORD_STATUS_CODES["OK"], KEYWORD_STATUS_CODES["BAD_REQUEST"]], \
...
#     def test_get_keywords_list_with_negative_limit(self):
#         """Тест получения списка ключевых слов с отрицательным лимитом"""
#         params = {'limit': -1}
...
#     def test_get_keywords_list_with_invalid_page(self):
#         """Тест получения списка ключевых слов с неверной страницей"""
#         params = {'page': -1}
...
#     def test_get_keywords_list_with_large_page(self):
#         """Тест получения списка ключевых слов с большой страницей"""
#         params = {'page': 999999}
...
#     def test_get_keywords_list_response_structure(self):
#         """Тест структуры ответа списка ключевых слов"""
#         response = KeywordsAPI.get_keywords_list()
...
#         if json_data['data']:
#             first_keyword = json_data['data'][0]
#             expected_fields = ['id', 'name', 'project', 'created_by', 'updated_by', 'created_at', 'updated_at']
            
#             for field in expected_fields:
#                 assert field in first_keyword, f"Отсутствует поле {field} в структуре ключевого слова"
            
#             # Проверяем типы данных
#             assert isinstance(first_keyword['id'], int), "ID должен быть числом"
#             assert isinstance(first_keyword['name'], str), "Название должно быть строкой"
        
#         print("Структура ответа соответствует ожиданиям")

#     def test_get_keywords_list_with_name_filter(self):
#         """Тест получения списка ключевых слов с фильтром по названию"""
#         # Сначала создаем тестовое ключевое слово
#         from tests_microservices.utils.incidents.keywords_schemas import VALID_KEYWORD_DATA
#         test_data = get_unique_keyword_data(VALID_KEYWORD_DATA)
#         create_response = KeywordsAPI.create_keyword(test_data)
...
#             print(f"Фильтр по названию работает. Найдено ключевых слов: {len(json_data['data'])}")
#         else:
#             print("Не удалось создать тестовое ключевое слово для проверки фильтра")

#     @handle_server_timeout
#     def test_get_keywords_list_with_multiple_params(self):
#         """Тест получения списка ключевых слов с несколькими параметрами"""
#         params = {
#             'page': 1,
#             'limit': 10,
#             'name': 'ключевое'
...
#     @handle_server_timeout
#     def test_get_keywords_list_performance(self):
#         """Тест производительности получения списка ключевых слов"""
#         import time
